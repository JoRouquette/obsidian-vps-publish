...
export class MarkdownItRenderer implements MarkdownRendererPort {
  ...
  private processLinkElement(
    ...
    if (href?.startsWith('#')) {
      ...
      if (rawFragment) {
        const slugified = this.headingSlugger.slugify(rawFragment);
        $link.attr('href', '#' + slugified);
      }
      return;
    }
  ...
  private translateToRoutedPathWithValidation(
    ...
    if (matchingPage) {
      // Slugify anchor fragment to match heading IDs generated by markdown-it-anchor
      const slugifiedAnchor = anchor ? '#' + this.headingSlugger.slugify(anchor.substring(1)) : '';
      return {
        path: matchingPage.route + slugifiedAnchor,
        matchedPage: matchingPage,
      };
    }

    // If no match found, return original path with leading slash but no matched page
    // Still slugify anchor to match heading IDs
    if (anchor) {
      const slugifiedAnchor = '#' + this.headingSlugger.slugify(anchor.substring(1));
      const base = basePath.startsWith('/') ? basePath : '/' + basePath;
      return {
        path: base + slugifiedAnchor,
