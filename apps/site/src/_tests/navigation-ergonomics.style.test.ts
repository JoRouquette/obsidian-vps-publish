...
describe('Site navigation ergonomics styles', () => {
  ...
  it('keeps mobile breadcrumbs as a compact orientation pattern instead of a scroll strip', () => {
    ...
    expect(source).toContain('--topbar-secondary-hover-bg:');
    expect(source).toContain('padding: 0.25rem var(--interactive-secondary-padding-inline);');
    expect(source).toContain('border-radius: var(--interactive-secondary-pill-radius);');
  ...
  it('gives explorer search and rows more visual rhythm and comfortable interactive states', () => {
    ...
    expect(source).toContain('--row-radius: var(--interactive-secondary-soft-radius);');
    ...
    expect(source).toContain('border-radius: var(--interactive-secondary-soft-radius);');
    expect(source).toMatch(/--row-h:\s*var\(\s*--interactive-secondary-touch-target\s*\);/);
  });
  ...
  it('keeps mobile overlays integrated as rounded panels instead of raw edge-to-edge sheets', () => {
    ...
    expect(source).toContain(
      '--shell-interactive-pill-radius: var(--interactive-secondary-pill-radius);'
    );
    expect(source).toContain(
      '--shell-interactive-touch-target: var(--interactive-secondary-touch-target);'
    );
